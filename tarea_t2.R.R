...
# --- Cifras de apoyo para la interpretación ---------------------------------

# Promedio del resto centro-sur, para la comparación de dos grupos
# (promedio sobre las 28 personas, no promedio de los 3 promedios regionales):
base |>
  group_by(zona) |>
  summarise(n = n(), ipe_prom = mean(ing_por_educ))
# Ñuble 12 -> 58.136 ; Resto centro-sur 28 -> 50.303

max(resultado$ipe_prom) - min(resultado$ipe_prom)   # 10.690 (Ñuble - Maule)

# Composición por sector, para respaldar el punto 3:
table(base$region, base$sector)
# Ñuble: 5 de 12 en Servicios y 0 en Comercio

# INTERPRETACIÓN

# 1. En Ñuble cada año de educación rinde $58.136, $10.690 más que en Maule
#    (el más bajo) y $7.800 más que el resto del centro-sur ($50.303).
# 2. La ventaja se da en ambos niveles comparables, pero se concentra en Sin
#    media completa (+$12.867) más que en Superior (+$4.882).
# 3. Parte puede venir de la medición: dividir por años de educación favorece a
#    quien tiene menos. El sector también podría influir (Ñuble: 5/12 en
#    Servicios, 0 en Comercio).
# 4. Limitación: solo 40 casos y sin factor de expansión; describe a estas
#    personas, no a la población.
# 5. No permite afirmar causalidad; con esta n, la diferencia podría ser ruido.
